...
export default function App() {
  ...
  const [fileContent, setFileContent] = useState('Այս փաստաթուղթը պատրաստ է տպագրության։ Այն ցուցադրվում է էկրանին որպես թուղթ, որից հետո կարող եք անմիջապես ուղղել տպիչին։')
  const [isPrinting, setIsPrinting] = useState(false)

  // Օգտատիրոջ վիճակը (Google-ից ստացվող տվյալների համար)
  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        // Վերցնում ենք օգտատիրոջ տվյալները Google-ի API-ից access_token-ի միջոցով
    ...
    <div className="app-shell">
      ...
      <main className="content">
            ...
            <section id="overview" className="hero-section">
              <div className="hero-copy">
                ...
                <div className="hero-actions">
                  <button className="primary-button" onClick={() => setActiveTab('print-tool')} style={{ border: 'none', cursor: 'pointer' }}>
        ...
        {activeTab === 'print-tool' && (
          <section id="print-tool" className="print-workspace fade-in" style={{ padding: '32px', display: 'flex', flexDirection: 'column', gap: '24px', maxWidth: '900px', margin: '0 auto', width: '100%' }}>
            <div style={{ color: '#111', display: 'flex', flexDirection: 'column', justifyContent: 'space-between', position: 'relative' }}>
              <div>
                <h3 style={{ fontSize: '20px', wordBreak: 'break-all' }}>{fileName}</h3>
              </div>
            </div>
          </section>
        )}
